...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h1 class="mb-0" style="color: #e85d1f;">Nuevo Pedido</h1>
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.pedidos.store') }}">
                ...
                <div class="mb-3">
                    ...
                    <textarea name="notas" id="notas" class="form-control @error('notas') is-invalid @enderror" maxlength="255">{{ old('notas') }}</textarea>
                    @error('notas')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>

                <button type="submit" class="btn btn-naranja">Guardar Pedido</button>
            </form>
        </div>
    </div>
@endsection
