{{-- filepath: f:\cafeteria2\resources\views\admin\item_pedidos\edit.blade.php --}}
@extends('layouts.admin')

@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h1 class="mb-0" style="color: #e85d1f;">Editar Ítem de Pedido</h1>
    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <form method="POST" action="{{ route('item_pedidos.update', $itemPedido->id) }}">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <select name="pedido_id" id="pedido_id" class="form-select @error('pedido_id') is-invalid @enderror" required>
                        ...
                        @foreach($pedidos as $pedido)
                            <option value="{{ $pedido->id }}" {{ old('pedido_id', $itemPedido->pedido_id) == $pedido->id ? 'selected' : '' }}>
                                Pedido #{{ $pedido->id }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="producto_id" id="producto_id" class="form-select @error('producto_id') is-invalid @enderror" required>
                        ...
                        @foreach($productos as $producto)
                            <option value="{{ $producto->id }}" {{ old('producto_id', $itemPedido->producto_id) == $producto->id ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" min="1" name="cantidad" id="cantidad"
                        ...
                        value="{{ old('cantidad', $itemPedido->cantidad) }}" required>
                    @error('cantidad')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" step="0.01" min="0" name="precio_unitario" id="precio_unitario"
                        ...
                        value="{{ old('precio_unitario', $itemPedido->precio_unitario) }}" required>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="solicitudes_especiales" id="solicitudes_especiales"
                        ...
                        value="{{ old('solicitudes_especiales', $itemPedido->solicitudes_especiales) }}" maxlength="255">
                    @error('solicitudes_especiales')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>

                <button type="submit" class="btn btn-naranja">Actualizar Ítem</button>
            </form>
        </div>
    </div>
@endsection
