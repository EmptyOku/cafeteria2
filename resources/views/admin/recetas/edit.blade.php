...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h1 class="mb-0" style="color: #e85d1f;">Editar Receta</h1>
            ...
            <form method="POST" action="{{ route('recetas.update', $receta->id) }}">
                    ...
                    <select name="insumo_id" id="insumo_id" class="form-select @error('insumo_id') is-invalid @enderror" required>
                        ...
                        @foreach($insumos as $insumo)
                            <option value="{{ $insumo->id }}"
                                {{ old('insumo_id', $receta->insumo_id) == $insumo->id ? 'selected' : '' }}>
                                {{ $insumo->producto }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" step="0.01" min="0" name="cantidad" id="cantidad"
                        ...
                        value="{{ old('cantidad', $receta->cantidad) }}" required>
                    @error('cantidad')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="instrucciones" id="instrucciones" class="form-control @error('instrucciones') is-invalid @enderror"
                        maxlength="500">{{ old('instrucciones', $receta->instrucciones) }}</textarea>
                    @error('instrucciones')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>

                <button type="submit" class="btn btn-naranja">Actualizar Receta</button>
            </form>
        </div>
    </div>
@endsection
